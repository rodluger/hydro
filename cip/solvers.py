...
class Spherical(object):
  ...
  def __init__(self, r, rho, u, p, dt = 0.001, gamma = 1.4, a = 0.65, CSL2 = False,
    ...
    :param r: The independent coordinate (radial) array
    :param rho: The density array
    :param u: The velocity array
    :param p: The pressure array
    :param dt: The timestep (must be small enough to satisfy the CFL condition!)
    ...
    assert CSL2 is False, "CSL2 solver not yet implemented."
    ...
    # Compute dr array (forward difference, r[i+1] - r[i])
  ...
  def InitPlot(self):
    ...
    self.ax[2].set_ylabel('Velocity', fontsize = 14)
    self.ax[3].set_ylabel('Energy', fontsize = 14)
    self.ax[2].set_xlabel('Radius', fontsize = 14)
    self.ax[3].set_xlabel('Radius', fontsize = 14)
    self.ax[0].legend(loc = 'upper right')
  ...
  def RhoCSL2(self):
    ...
    # Negative velocity: iup = i + 1; icell = i + 1/2
    drm = self.r[self.IBEG+1:self.IEND+1] - self.r[self.IBEG:self.IEND]
    A1m = (self.rho[self.IBEG:self.IEND] + self.rho[self.IBEG+1:self.IEND+1]) / drm ** 2 - \
          2 * self.eta[self.HBEG+1:self.HEND] / drm ** 3
    A2m = -(2 * self.rho[self.IBEG:self.IEND] + self.rho[self.IBEG+1:self.IEND+1]) / drm + \
          3 * self.eta[self.HBEG+1:self.HEND] / drm ** 2
    
    # Positive velocity: iup = i - 1; icell = i - 1/2
    drp = self.r[self.IBEG-1:self.IEND-1] - self.r[self.IBEG:self.IEND]
    A1p = (self.rho[self.IBEG:self.IEND] + self.rho[self.IBEG-1:self.IEND-1]) / drp ** 2 + \
          2 * self.eta[self.HBEG:self.HEND-1] / drp ** 3
    A2p = -(2 * self.rho[self.IBEG:self.IEND] + self.rho[self.IBEG-1:self.IEND-1]) / drp - \
          3 * self.eta[self.HBEG:self.HEND-1] / drp ** 2
    ...
    G[self.IBEG:self.IEND] = -rhostar[self.IBEG:self.IEND] * \
                            (self.u[self.HBEG+1:self.HEND] - 
                            self.u[self.HBEG:self.HEND-1]) / self.drh[self.HBEG:self.IEND-1]
    ...
    xi = -uav * self.dt
    if uav < 0:
      dr = self.r[self.IBEG] - self.r[self.IBEG-1]
      A1 = (self.rho[self.IBEG-1] + self.rho[self.IBEG]) / dr ** 2 - 2 * self.eta[self.HBEG] / dr ** 3
      A2 = -(2 * self.rho[self.IBEG-1] + self.rho[self.IBEG]) / dr + 3 * self.eta[self.HBEG] / dr ** 2
    else:
      dr = self.r[self.IBEG-2] - self.r[self.IBEG-1]
      A1 = (self.rho[self.IBEG-1] + self.rho[self.IBEG-2]) / dr ** 2 + 2 * self.eta[self.HBEG-1] / dr ** 3
      A2 = -(2 * self.rho[self.IBEG-1] + self.rho[self.IBEG-2]) / dr - 3 * self.eta[self.HBEG-1] / dr ** 2
    deltaeta[self.IBEG-1] = -(A1 * xi ** 3 + A2 * xi ** 2 + self.rho[self.IBEG-1] * xi)
    ...
    xi = -uav * self.dt
    if uav < 0:
      dr = self.r[self.IEND+1] - self.r[self.IEND]
      A1 = (self.rho[self.IEND] + self.rho[self.IEND+1]) / dr ** 2 - 2 * self.eta[self.HEND] / dr ** 3
      A2 = -(2 * self.rho[self.IEND] + self.rho[self.IEND+1]) / dr + 3 * self.eta[self.HEND] / dr ** 2
    else:
      dr = self.r[self.IEND-1] - self.r[self.IEND]
      A1 = (self.rho[self.IEND] + self.rho[self.IEND-1]) / dr ** 2 + 2 * self.eta[self.HEND-1] / dr ** 3
      A2 = -(2 * self.rho[self.IEND] + self.rho[self.IEND-1]) / dr - 3 * self.eta[self.HEND-1] / dr ** 2
    deltaeta[self.IEND] = -(A1 * xi ** 3 + A2 * xi ** 2 + self.rho[self.IEND] * xi)
  ...
  def RhoStar(self):
    '''
    This is Equation (26) in Yabe and Aoki (1991) modified for spherical coordinates,
    Equation (30).
    
    TODO: Check that I've done this correctly. There may be subtleties with the
    radius half grid.
    ...
    # Radius above (half step)
    rhp = self.rh[self.HBEG+1:self.HEND]
    
    # Radius below (half step)
    rhm = self.rh[self.HBEG:self.HEND-1]
    
    # Delta r (half step)
    drh = self.drh[self.HBEG:self.HEND-1]
    
    # This is the first term in the G vector at the bottom of page 229
    drhodt[self.IBEG:self.IEND] = -(rho / r ** 2) * (rhp ** 2 * up - rhm ** 2 * um) / drh
    
    return self.rho + self.dt * drhodt

  def UStar(self):
    '''
    Same as in Cartesian coordinates.
    
    '''
    
    dudt = np.zeros_like(self.u)
    dudt[self.HBEG:self.HEND] = -((self.p[self.IBEG:self.IEND+1] - 
                                 self.p[self.IBEG-1:self.IEND]) / self.dr[self.IBEG:self.IEND+1]) / \
                                 (0.5 * (self.rho[self.IBEG:self.IEND+1] + 
                                 self.rho[self.IBEG-1:self.IEND]))
    return self.u + self.dt * dudt
  ...
  def EStar(self):
    '''
    This is Equation (28) in Yabe and Aoki (1991) modified for spherical coordinates,
    Equation (32).
  ...
  def CIP0(self, f, fprime, half = False):
    ...
    if not half:
      ...
      uav = self.u[self.HBEG:self.HEND]
      xi = -uav * self.dt
      drm = self.r[self.HBEG+1:self.HEND+1] - self.r[self.HBEG:self.HEND]
      drp = self.r[self.HBEG-1:self.HEND-1] - self.r[self.HBEG:self.HEND]

    # Negative velocity
    am = (fprime[BEG:END] + fprime[BEG+1:END+1]) / (drm ** 2) + \
          2 * (f[BEG:END] - f[BEG+1:END+1]) / (drm ** 3)
    bm = 3 * (f[BEG+1:END+1] - f[BEG:END]) / (drm ** 2) - \
        (2 * fprime[BEG:END] + fprime[BEG+1:END+1]) / drm
    
    # Positive velocity
    ap = (fprime[BEG:END] + fprime[BEG-1:END-1]) / (drp ** 2) + \
          2 * (f[BEG:END] - f[BEG-1:END-1]) / (drp ** 3)
    bp = 3 * (f[BEG-1:END-1] - f[BEG:END]) / (drp ** 2) - \
        (2 * fprime[BEG:END] + fprime[BEG-1:END-1]) / drp
  ...
  def NumVisc(self):
    ...
    visc = np.zeros_like(self.rho)
    du = (self.rh[self.HBEG+1:self.HEND] * self.u[self.HBEG+1:self.HEND] - self.rh[self.HBEG:self.HEND-1] * self.u[self.HBEG:self.HEND-1])
    c = np.sqrt(self.gamma * self.p / self.rho)[self.IBEG:self.IEND]
    visc[self.IBEG:self.IEND] =  np.where(du < 0,
                                  self.a * (-self.rho[self.IBEG:self.IEND] * c * du + 
                                  0.5 * (self.gamma + 1) * self.rho[self.IBEG:self.IEND] * du ** 2),
