...
class _analytic(object):
  '''
  The analytic solution to the Sod shock tube problem, borrowed from
  https://gitlab.com/fantaz/simple_shock_tube_calculator
  
  '''
    
  def __init__(self, time, gamma = 1.4, left_state=(1, 1, 0), right_state=(0.1, 0.125, 0.), geometry=(0., 2., 1.)):
      '''

      '''
      
      # Compute
      positions, regions, values = self.solve(left_state=left_state, right_state=right_state,
                                              geometry=geometry, t=time, gamma = gamma)
      ...
      self.p = values['p']
      self.u = values['u']
      self.e = values['p'] / (gamma - 1) / values['rho']
  
  def shock_tube_function(self, p4, p1, p5, rho1, rho5, gamma):
    """
    Shock tube equation
  ...
  def calculate_regions(self, pl, ul, rhol, pr, ur, rhor, gamma=1.4):
    """
    Compute regions
    :rtype : tuple
    ...
    p4 = fsolve(self.shock_tube_function, p1, (p1, p5, rho1, rho5, gamma))[0]
  ...
  def calc_positions(self, pl, pr, region1, region3, w, xi, t, gamma):
    """
  ...
  def region_states(self, pl, pr, region1, region3, region4, region5):
    """
  ...
  def create_arrays(self, pl, pr, xl, xr, positions, state1, state3, state4, state5, npts, gamma, t, xi):
    """
    ...
    gp1 = gamma + 1.
    dx = (xr-xl)/(npts-1)
    x_arr = np.arange(xl/dx, xr/dx) * dx
  ...
  def solve(self, left_state, right_state, geometry, t, gamma=1.4, npts=500):
    """
    ...
    # calculate regions
    region1, region3, region4, region5, w = self.calculate_regions(pl, ul, rhol, pr, ur, rhor, gamma)
    regions = self.region_states(pl, pr, region1, region3, region4, region5)

    # calculate positions
    x_positions = self.calc_positions(pl, pr, region1, region3, w, xi, t, gamma)
    ...
    # create arrays
    x, p, rho, u = self.create_arrays(pl, pr, xl, xr, x_positions,
...
class Sod(Cartesian):
  ...
  def __init__(self, npoints = 200, xmin = 0, xmax = 2, gridpower = 1, **kwargs):
    ...
    x = np.linspace(xmin ** gridpower, xmax ** gridpower, npoints) ** (1. / gridpower)

    # Initial profiles for the Sod shock tube
    midpt = np.argmin(np.abs(x - 0.5 * (xmax - xmin)))
    rho = np.ones(npoints)
    rho[midpt:] = 0.125
    ...
    # Initialize
    super(Sod, self).__init__(x, rho, u, p, rholim = rholim, ulim = ulim, plim = plim, elim = elim, **kwargs)
  ...
  def Animate(self, **kwargs):
    ...
    assert kwargs.get('tend', 0.276) <= 0.5, "The Sod shock tube solution is only valid up to t = 0.5"
    super(Sod, self).Animate(**kwargs)
  ...
  def InitPlot(self):
    ...
    super(Sod, self).InitPlot()
    
    # Analytic solution
    analytic = _analytic(self.time, gamma = self.gamma)
    self.xa = analytic.x
    self.rhoa = analytic.rho
    self.pa = analytic.p
    self.ua = analytic.u
    self.ea = analytic.e
  ...
  def UpdateFig(self, *args):
    ...
    # Call the parent function
    return super(Sod, self).UpdateFig(*args)