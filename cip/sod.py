...
def calculate_regions(pl, ul, rhol, pr, ur, rhor, gamma=1.4):
  ...
  p4 = fsolve(shock_tube_function, p1, (p1, p5, rho1, rho5, gamma))[0]
...
def Sod(time, gamma = 1.4, left_state=(1, 1, 0), right_state=(0.1, 0.125, 0.),
        geometry=(0., 2., 1.)):
  '''
  
  '''
  
  positions, regions, values = solve(left_state=left_state, right_state=right_state,
                                     geometry=geometry, t=time, gamma = gamma)

  return values['x'], values['rho'], values['p'], values['u'], values['p'] / (gamma - 1) / values['rho']